...
Date: June 2023
----------------------------------------
Modified by: Kangwei Liao on Mar. 2024
"""
...
def load_tokenize_data(args):
    ...
    if os.path.exists(args.cache_data):
        ...
        def filter_entries(examples):
            # Filter based on scores
            return examples['question_score'] >= 30 and examples['answer_score'] >= 5
        ...
        def preprocess_function(examples):
            ...
            target = [clean_text(a) for a in examples['answer_body']]
        ...
        datasets = load_dataset("koutch/stackoverflow_python", split="train")
        tokenizer = AutoTokenizer.from_pretrained(args.load)

        datasets = datasets.filter(filter_entries)  # Filter based on scores
        ...
        print(f'  ==> Loaded {len(train_data)} samples from default dataset')
...
def main(args):
    argsdict = vars(args)
    print("Applying following arguments:")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max-target-len', default=256, type=int)
    parser.add_argument('--cache-data', default='cache_data/default/', type=str)
